...
class YOG_522e:
    ...
    tags = {
        GameTag.CARDTYPE: CardType.ENCHANTMENT,
        enums.ABILITY_USED_THIS_TURN: 0,  # 0 = 未使用，1 = 已使用
    }
    ...
    events = Play(CONTROLLER, SPELL).on(
        lambda self, player, played_card, target: [
            ...
            # 标记为已使用并销毁
            SetTags(SELF, {enums.ABILITY_USED_THIS_TURN: 1}),
            Destroy(SELF)
        ]
    )
...
class TTN_800e:
    ...
    tags = {
        GameTag.CARDTYPE: CardType.ENCHANTMENT,
        enums.ABILITY_USED_THIS_TURN: 0,  # 0 = 未使用法术，1 = 已使用法术
    }
    ...
    events = [
        ...
        # 监听法术施放，设置标记
        Play(CONTROLLER, SPELL).on(SetTags(SELF, {enums.ABILITY_USED_THIS_TURN: 1}))
    ]
