...
shaman_cards.sort(key=lambda c: (rarity_order.get(c.get('rarity', ''), 5), c.get('cost', 0)))

print(f"找到 {len(shaman_cards)} 张 Shaman 卡牌\n")
print("=" * 80)

for card in shaman_cards:
    print(f"\nID: {card['id']}")
...
# 保存到文件
with open('shaman_cards_data.json', 'w', encoding='utf-8') as f:
    json.dump(shaman_cards, f, ensure_ascii=False, indent=2)

with open('shaman_cards_data.txt', 'w', encoding='utf-8') as f:
    for card in shaman_cards:
        f.write(f"\n{'=' * 80}\n")
...
print(f"\n数据已保存到 shaman_cards_data.json 和 shaman_cards_data.txt")
