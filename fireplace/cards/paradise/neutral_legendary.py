...
class VAC_702:
    ...
    def play(self):
        # 让玩家选择一个宝藏
        treasures = [self.controller.card(tid) for tid in VAC_702.TREASURES]
        ...
        if chosen:
            ...
            # 将其余3个宝藏洗入牌库
            for treasure_id in VAC_702.TREASURES:
...
class VAC_959:
    ...
    def play(self):
        # 发现一个护符 (只显示真品)
        real_amulets = [pair[0] for pair in VAC_959.AMULETS]
        ...
        if discovered:
            ...
            # 找到对应的假货给对手
            for real, fake in VAC_959.AMULETS:
                if real == chosen_id:
    ...
    def play(self):
        ...
        if chosen_tour:
            ...
            if tour_id == "WORK_027t1":
                card_pool = WORK_027.EXTRAVAGANT_TOUR
            elif tour_id == "WORK_027t2":
                card_pool = WORK_027.HECTIC_TOUR
            else:
                card_pool = WORK_027.MODEST_TOUR
