...
# ============================================================
# 语义化脚本数据标签（替代通用的 TAG_SCRIPT_DATA_NUM_*）
# ============================================================

# 深渊诅咒机制 - 沉没之城（2022年4月）
ABYSSAL_CURSE_LEVEL = -82  # 深渊诅咒当前等级（伤害值）

# 成长计数器机制 - 泰坦诸神（2023年8月）
GROWTH_COUNTER = -83  # 成长类卡牌的剩余成长次数

# 使用状态追踪 - 泰坦诸神（2023年8月）
ABILITY_USED_THIS_TURN = -84  # 本回合是否已使用某能力（0=未使用，1=已使用）

# 剩余回合数机制 - 失落之城（2025年7月）
TURNS_REMAINING = -85  # 效果的剩余回合数

# 翻倍计数器 - 泰坦诸神（2023年8月）
POWER_MULTIPLIER = -86  # 法术翻倍次数（指数）

# 触发计数器 - 通用
TRIGGER_COUNT = -87  # 效果触发次数计数

# 法术施放状态追踪 - 泰坦诸神（2023年8月）
SPELL_CAST_THIS_TURN = -88  # 本回合是否已施放法术（0=未施放，1=已施放）

# 对手打出卡牌计数 - 纳斯利亚堡（2022年8月）
OPPONENT_CARDS_PLAYED_COUNT = -89  # 对手本回合打出的卡牌数量（用于 MAW_010 等奥秘）

# 圣盾吸收次数 - 深入翡翠梦境（2025年3月）
DIVINE_SHIELD_ABSORB_COUNT = -90  # 圣盾需要承受的剩余伤害次数（用于 EDR_258 托雷斯等）

# 地标使用计数 - 深入翡翠梦境（2025年3月）
LOCATION_USE_COUNT = -91  # 地标激活的次数（用于 FIR_907 阿梅达希尔等）

# 亡语抽牌数 - 传奇音乐节（2023年4月）
DEATHRATTLE_DRAW_COUNT = -92  # 亡语抽牌的数量（用于 ETC_405 战刃吉他等）

# 亡语召唤费用 - 传奇音乐节（2023年4月）
DEATHRATTLE_SUMMON_COST = -93  # 亡语召唤随从的费用（用于 ETC_832 丛林弹唱琴等）

# 费用追踪值 - 传奇音乐节（2023年4月）
TRACKED_COST_VALUE = -94  # 追踪的卡牌费用值（用于 ETC_422 计拍侏儒等）

# 法力水晶获取量 - 传奇音乐节（2023年4月）
MANA_CRYSTAL_GAIN_AMOUNT = -95  # 获取/复原的法力水晶数量（用于 ETC_518 搓盘机等）

# 法术减费量 - 传奇音乐节（2023年4月）
SPELL_COST_REDUCTION = -96  # 法术费用减少的数量（用于 ETC_813 爵士贝斯等）

# 亡语伤害量 - 传奇音乐节（2023年4月）
DEATHRATTLE_DAMAGE_AMOUNT = -97  # 亡语造成的伤害量（用于 ETC_520 科多兽皮组鼓等）

# 属性加成量 - 传奇音乐节（2023年4月）
BUFF_STAT_AMOUNT = -98  # 攻击力/生命值加成的数量（用于 ETC_317 迪斯科战槌等）

# 子弹计数 - 决战荒芜之地（2023年11月）
BULLET_COUNT = -99  # 储存的子弹数量（用于 WW_431 枪尾蛇等）

# 卡牌追踪索引 - 暗月马戏团（2020年11月）
CARD_TRACKER_INDEX = -100  # 卡牌追踪索引（用于 DMF_249 等多卡追踪场景）
